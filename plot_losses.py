...
for current_algo in algorithms:
    ...
    for dir_name in checkpoint_dirs:
        ...
        # Only process directories that contain the current algorithm name
        if current_algo not in dir_name.lower():
            continue
        
        # Load train losses
        train_files = [f for f in os.listdir(dir_path) if f.startswith('train_loss_list')]
        for train_file in train_files:
            train_path = os.path.join(dir_path, train_file)
            train_data = load_pickle(train_path)
            exp_name = extract_experiment_name(dir_name)
            if exp_name not in train_losses:
                train_losses[exp_name] = []
            train_losses[exp_name].extend(train_data)
        
        # Load test losses
        test_files = [f for f in os.listdir(dir_path) if f.startswith('test_loss_list')]
        for test_file in test_files:
            test_path = os.path.join(dir_path, test_file)
            test_data = load_pickle(test_path)
            exp_name = extract_experiment_name(dir_name)
            if exp_name not in test_losses:
                test_losses[exp_name] = []
            test_losses[exp_name].extend(test_data)
    
    # Calculate mean test loss for each experiment
    mean_test_losses = {exp: np.mean(losses) for exp, losses in test_losses.items()}
    
    # Sort experiments based on mean test loss (descending order)
    sorted_experiments = sorted(mean_test_losses, key=mean_test_losses.get, reverse=True)
    
    # Prepare data for plotting
    plot_data = []
    for exp_name in sorted_experiments:
        if exp_name in train_losses:
            plot_data.extend([(exp_name, 'Train', loss) for loss in train_losses[exp_name]])
        if exp_name in test_losses:
            plot_data.extend([(exp_name, 'Test', loss) for loss in test_losses[exp_name]])
    
    df_plot = pd.DataFrame(plot_data, columns=['Experiment', 'Type', 'Loss'])
    
    # Create box plot only if there's data to plot
    if not df_plot.empty:
        plt.figure(figsize=(15, 8))
        sns.boxplot(x='Experiment', y='Loss', hue='Type', data=df_plot, order=sorted_experiments)
        plt.title(f"Train and Test Losses for {current_algo.upper()}", fontsize=16, fontweight='bold')
        plt.xlabel("Experiment", fontsize=12)
        plt.ylabel("Loss", fontsize=12)
        plt.xticks(rotation=45, ha='right')
        plt.legend(title='Loss Type')
        plt.tight_layout()
        plt.savefig(f'loss_comparison_{current_algo}.png', dpi=300, bbox_inches='tight')
        plt.close()
        
        print(f"Loss comparison plot saved as 'loss_comparison_{current_algo}.png'")
    else:
        print(f"No data found for {current_algo}. Skipping plot creation.")
    ...
    for dir_name in checkpoint_dirs:
        ...
        if exp_name not in results[current_algo]:
            results[current_algo][exp_name] = {'train': [], 'test': []}
        ...
        for train_file in train_files:
            train_path = os.path.join(dir_path, train_file)
            train_data = load_pickle(train_path)
            results[current_algo][exp_name]['train'].extend(train_data)
        ...
        for test_file in test_files:
            test_path = os.path.join(dir_path, test_file)
            test_data = load_pickle(test_path)
            results[current_algo][exp_name]['test'].extend(test_data)

    # Create a list to store table rows
    table_data = []

    # Process the results and create table rows
    for algo, experiments in results.items():
        for exp, data in experiments.items():
            train_mean = np.mean(data['train']) if data['train'] else np.nan
            train_std = np.std(data['train']) if data['train'] else np.nan
            test_mean = np.mean(data['test']) if data['test'] else np.nan
            test_std = np.std(data['test']) if data['test'] else np.nan
    ...
    # Create the table
    headers = ["Algorithm", "Experiment", "Train Loss (Mean ± Std)", "Test Loss (Mean ± Std)"]
    ...
    with open("loss_summary_table.txt", "w") as f:
        f.write(table)