...
for current_algo in algorithms:
    ...
    # Set the current algorithm flag to True
    if current_algo == 'a2c':
        if_using_a2c = True
    ...
    # Update algos_included
    algos_included = f'_{current_algo}'

    print(f"Running with {current_algo.upper()} algorithm")
    ...
    algos_included = ''
    if if_using_a2c: algos_included += '_a2c'
    if if_using_ddpg: algos_included += '_ddpg'
    if if_using_ppo: algos_included += '_ppo'
    if if_using_td3: algos_included += '_td3'
    if if_using_sac: algos_included += '_sac'
    if include_ensemble: algos_included += '_ensemble'
    ...
    plt.savefig(f'backtest_result_from_checkpoints_with_djia_and_error_bands_train_env_{current_algo}.png', dpi=300, bbox_inches='tight')
    plt.close()

    print(f"Processed {len(experiment_groups)} unique experiments.")
    print(f"Results saved as 'backtest_result_from_checkpoints_with_djia_and_error_bands_train_env_{current_algo}.png'")
    # Remove CSV saving
