...
def train(config: TrainConfig, args):
    ...
    config.group = config.env + "-td3_bc" + "-ratio=" + str(config.sample_ratio)
    ...
    for name, param in critic_1.named_parameters():
        param_size += param.numel()
    for name, param in critic_2.named_parameters():
        param_size += param.numel()
    print(f"Full parameters:", param_size)
    raise NotImplementedError
