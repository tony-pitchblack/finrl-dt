...
for algo in "${algos[@]}"; do
    ...
    if [ -z "$dataset_path" ] || [ -z "$test_trajectory" ]; then
    ...
    for seed in "${seeds[@]}"; do
        ...
        echo "Using test trajectory: $test_trajectory"
        
        CUDA_VISIBLE_DEVICES=$gpu ~/gits/FinRL-Tutorials/.conda/bin/python iql.py \
            --seed $seed \
            --drl_algo $algo \
